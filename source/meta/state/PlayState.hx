...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (!FlxG.save.data.foundDeadMeme && curSong == 'dead-meme') {
			FlxG.save.data.foundDeadMeme = true;
		}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		songDetails = CoolUtil.dashToSpace(SONG.song) + ' - ' + CoolUtil.difficultyFromNumber(storyDifficulty) + credits;
	...
	override function stepHit()
	{
		...
		if (curSong == 'Nerves-But-Bam-Fucking-Ruined-It') {
			switch (curStep) {
				case 28:
					funnis = new FlxSprite().loadGraphic(Paths.returnGraphic('bam shit/eas'));
					funnis.x = dadOpponent.x;
					funnis.x = dadOpponent.y;
					add(funnis);
					dadOpponent.visible = false;
					trace('eas time bois');
				case 243:
					trace('random sounds');
					funnis = new FlxSprite().loadGraphic(Paths.returnGraphic('bam shit/maxresdefault'));
					funnis.x = dadOpponent.x;
					funnis.x = dadOpponent.y;
					add(funnis);
					dadOpponent.visible = false;
				case 339:
					trace('random sounds2');
					funnis = new FlxSprite().loadGraphic(Paths.returnGraphic('bam shit/maxresdefault'));
					funnis.x = dadOpponent.x;
					funnis.x = dadOpponent.y;
					add(funnis);
				...
				case 404:
					trace('IMPOSTOR');
					funnis = new FlxSprite().loadGraphic(Paths.returnGraphic('bam shit/The_Impostor'));
					funnis.x = dadOpponent.x;
					funnis.x = dadOpponent.y;
					add(funnis);
					dadOpponent.visible = false;
				case 620:
					trace('random sounds3');
					funnis = new FlxSprite().loadGraphic(Paths.returnGraphic('bam shit/maxresdefault'));
					funnis.x = dadOpponent.x;
					funnis.x = dadOpponent.y;
					add(funnis);
				...
				case 732:
					trace('pain');
					funnis = new FlxSprite().loadGraphic(Paths.returnGraphic('bam shit/download'));
					funnis.x = dadOpponent.x;
					funnis.x = dadOpponent.y;
					add(funnis);
					dadOpponent.visible = false;
